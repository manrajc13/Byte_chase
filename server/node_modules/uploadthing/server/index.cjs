...
const extractRouterConfig$1 = (router)=>Effect__namespace.forEach(shared.objectKeys(router), (slug)=>Effect__namespace.map(shared.fillInputRouteConfig(router[slug].routerConfig), (config)=>({
                slug,
                config
            })));
...
const uploadFile = (file, opts)=>Effect__namespace.gen(function*() {
        const presigned = yield* generatePresignedUrl(file, opts?.contentDisposition ?? "inline", opts?.acl).pipe(Effect__namespace.catchTag("ConfigError", ()=>Effect__namespace.fail({
                code: "INVALID_SERVER_CONFIG",
                message: "Failed to generate presigned URL"
            })));
        const response = yield* uploadWithoutProgress(file, presigned).pipe(Effect__namespace.catchTag("ResponseError", (e)=>Effect__namespace.fail({
                code: "UPLOAD_FAILED",
                message: "Failed to upload file",
                data: e.toJSON()
            })));
...
const createRouteHandler = (opts)=>{
    return makeAdapterHandler((ev)=>Effect__namespace.succeed({
