...
Effect__namespace.gen(function*() {
...
const createRouteHandler = (opts)=>{
    const router = Effect__namespace.runSync(createRequestHandler(opts, "effect-platform"));
    return platform.HttpRouter.provideServiceEffect(router, AdapterArguments, Effect__namespace.map(platform.HttpServerRequest.HttpServerRequest, (serverRequest)=>({
            req: serverRequest,
            res: undefined,
            event: undefined
        }))).pipe(Effect__namespace.provide(Layer__namespace.setConfigProvider(configProvider(opts.config))));
};
